    ...
    else if (type == NETWORK_MODE_SIGN && network_safe(net) > 0)
    {
        ...
        if (recv_or_send == SIGN_SEND)
        {
            ...
            if ((pak_sign = buf_to_pak_sign(data)) != NULL)
            {
                ...
                if (core != NULL)
                {
                    core_add_sign(core->id, pak_sign->sign_ll, 0);
                }
            }
            if (pak_sign != NULL)
                free(pak_sign);
...
XLpak_ins * buf_to_pak_ins(uint8_t * buf){
    ...
    //operate_code
    data_get(buf,&p,&pak_ins->operate_code,sizeof(uint));
    printf("\nop code is:%d\n",pak_ins->operate_code);
    //INS
...
uint8_t * pak_ins_to_buf(XLpak_ins *pak_ins,int * size){
    ...
    //pak_size=SIZE_PAKINS_WITHOUT_INS; //数据包头部的大小
    ...
    data_add(data,&p,&pak_ins->operate_code,sizeof(uint));
    if(pak_ins->ins!=NULL)
...
uint8_t * pak_sign_to_buf(XLpak_sign * pak_sign,int * size){
    ...
    XLll * sign_ll=pak_sign->sign_ll;
    XLll_member * member=sign_ll->head;
    for(int i=0;i<sign_ll->member_num;i++){
        XLpak_signinfo * sign_info=(XLpak_signinfo*)member->data;
        pak_size+=sizeof(uint8_t);
        pak_size+=strlen(sign_info->name)+1;
        pak_size+=strlen(sign_info->name)+1;
        member=member->next;
    }
    printf("size:%d\n",pak_size);
    ...
    for(int i=0;i<sign_ll->member_num;i++){
        XLpak_signinfo * sign_info=(XLpak_signinfo*)member->data;
        data_add_str(data,&p,sign_info->name);
        data_add_str(data,&p,sign_info->type);
        member=member->next;
    }
...
int network_send_ins(XLsource * sender,XLsource *receiver,uint operate_code,INS * ins){
    ...
    pak_ins.operate_code=operate_code;
    pak_ins.ins=ins;
...
int network_send_sign(core_id_t core_id){
    ...
    pak_sign.sign_ll=ll_create(sizeof(XLpak_signinfo));
    ...
    for(int i=0;i<=event_ll->member_num;i++){
        ...
        if(event_now->sign.use==ENABLE){
            XLpak_signinfo new_info;
            new_info.name=event_now->sign.name;
            new_info.type=event_now->sign.type;
            ll_add_member_tail(pak_sign.sign_ll,&new_info,sizeof(XLpak_signinfo));
        }
...
int network_send_ins_to_sourcell(XLsource * sender,XLll * source_ll,INS * ins){
    if(sender==NULL||ins==NULL)return -1;
    XLll_member * member_now=source_ll->head;
    for(int i=0;i<source_ll->member_num;i++){
        network_send_ins(sender,(XLsource*)member_now->data,0,ins);
        member_now=member_now->next;
    }
    return 1;
}
